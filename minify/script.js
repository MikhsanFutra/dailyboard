...
// Minggu 9 -> Fungsi Edit Catatan & Validasi input
...
btnCariCuaca.addEventListener("click", () => {
  if (inputKota.value.trim() !== "") {
    ambilCuaca(inputKota.value.trim());v
  }
});
...
function aktifkanDragDrop() {
  ...
  list.ondrop = (e) => {
    ...
    const indexLama = daftarTugas.findIndex((t) => t.id === id);
    const targetElement = e.target.closest(".tugas-item");

    if (targetElement && indexLama !== -1) {
      const idTarget = Number(targetElement.dataset.id);
      const indexBaru = daftarTugas.findIndex((t) => t.id === idTarget);

      if (indexBaru !== -1) {
        const [tugasDipindah] = daftarTugas.splice(indexLama, 1);
        daftarTugas.splice(indexBaru, 0, tugasDipindah);
...
const btnToggleTema = document.createElement("button");
...
function renderTugasKustom(daftar = daftarTugas) {
  const list = document.getElementById("daftar-tugas");
  list.innerHTML = "";
